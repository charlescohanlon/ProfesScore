  ...
  return (
    <input
      ...
      onKeyDown={({ key, currentTarget: { value } }) =>
        key === "Enter"
          ? (outputRef.current!.value = value) // set val of slider to input val
          : null
      }
      defaultValue={defaultVal}
      className={
        "w-10 sm:w-14 lg:w-20 py-2 rounded-full font-Barlow text-brandGray bg-white text-center" +
        " hover:shadow-inputShadow focus:outline-none text-xs sm:text-sm md:text-md lg:text-lg xl:text-xl 2xl:text-2xl"
      }
    />
  );
};
